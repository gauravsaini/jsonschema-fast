...
from jsonschema_fast.validators import _VALIDATORS
...
def _find_suite():
    ...
    root = Path(jsonschema_fast.__file__).parent.parent / "json"
...
class _Test:
    ...
    def to_unittest_method(self, skip=lambda test: None, **kwargs):
        if self.valid:
            ...
            def fn(this):
                with this.assertRaises(jsonschema_fast.ValidationError):
                    self.validate(**kwargs)
        ...
        if reason is None or os.environ.get("JSON_SCHEMA_DEBUG", "0") != "0":
            return fn
        elif os.environ.get("JSON_SCHEMA_EXPECTED_FAILURES", "0") != "0":  # pragma: no cover
    ...
    def validate_ignoring_errors(self, Validator):  # pragma: no cover
        with suppress(jsonschema_fast.ValidationError):
