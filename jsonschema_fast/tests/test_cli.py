...
from jsonschema_fast import Draft4Validator, Draft202012Validator
from jsonschema_fast.exceptions import (
    SchemaError,
    ValidationError,
    _RefResolutionError,
)
from jsonschema_fast.validators import _LATEST_VERSION, validate

with warnings.catch_warnings():
    warnings.simplefilter("ignore")
    from jsonschema_fast import cli
...
class TestParser(TestCase):
    ...
    def test_find_validator_by_fully_qualified_object_name(self):
        arguments = cli.parse_args(
            [
                "--validator",
                "jsonschema_fast.tests.test_cli.TestParser.FakeValidator",
...
class TestCLIIntegration(TestCase):
    def test_license(self):
        try:
            our_metadata = metadata.metadata("jsonschema_fast")
        except metadata.PackageNotFoundError:
            our_metadata = metadata.metadata("jsonschema_fast-fast")
        self.assertEqual(our_metadata.get("License-Expression"), "MIT")

    def test_version(self):
        version = subprocess.check_output(
            [sys.executable, "-W", "ignore", "-m", "jsonschema_fast", "--version"],
            stderr=subprocess.STDOUT,
        )
        version = version.decode("utf-8").strip()
        try:
            expected_version = metadata.version("jsonschema_fast")
        except metadata.PackageNotFoundError:
            expected_version = metadata.version("jsonschema_fast-fast")
    ...
    def test_no_arguments_shows_usage_notes(self):
        ...
        output_for_help = subprocess.check_output(
            [sys.executable, "-m", "jsonschema_fast", "--help"],
            stderr=subprocess.STDOUT,
        )
