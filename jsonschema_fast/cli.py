"""
The ``jsonschema_fast`` command line.
"""
...
from jsonschema_fast.exceptions import SchemaError
from jsonschema_fast.validators import _RefResolver, validator_for

warnings.warn(
    (
        "The jsonschema_fast CLI is deprecated and will be removed in a future "
        "version. Please use check-jsonschema_fast instead, which can be installed "
        "from https://pypi.org/project/check-jsonschema_fast/"
    ),
    DeprecationWarning,
    stacklevel=2,
)
...
def _resolve_name_with_default(name):
    if "." not in name:
        name = "jsonschema_fast." + name
    return resolve_name(name)


def _get_version():
    try:
        return metadata.version("jsonschema_fast")
    except metadata.PackageNotFoundError:
        return metadata.version("jsonschema_fast-fast")
...
parser.add_argument(
    ...
    help="""
        ...
        validators that are registered with jsonschema_fast, simply the name
        of the class.
    """,
)
parser.add_argument(
